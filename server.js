...
wss.on("connection", (ws) => {
    ...
    ws.on("message", (raw) => {
        ...
        switch (msg.type) {
            ...
            case "move": {
                // Validierung: Spieler darf nicht zu weit springen
                const dx = Math.abs(msg.x - playerData.x);
                const dz = Math.abs(msg.z - playerData.z);
                if (dx > 5 || dz > 5) break; // Cheat-Schutz
                ...
                // Nur an andere Spieler weiterleiten
                broadcast({
                    type: "player_move",
                    id: playerId,
                    x: playerData.x,
...
setInterval(async () => {
    ...
    await supabase.from("mmo_players_log").upsert(
        snapshot.map(p => ({ player_id: p.id, data: p, updated_at: new Date().toISOString() }))
    ).catch(e => console.error("[DB] Spieler-Log Fehler:", e.message));
}, 30000);
